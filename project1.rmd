...
```{r}
with(df1,barplot(steps,xlab="date",ylab="steps each day"))
```
...
```{r}
with(interval,plot(interval,average,type="l"))
```
...
Make a hist plot 
```{r}
with(df2,barplot(steps,xlab="date",ylab="total steps per day"))
```
...
library(lattice)
finalplot<- with(finalsubset, xyplot(steps~interval|daytype,type="l"))
finalplot